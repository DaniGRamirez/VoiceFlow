...
from voiceflow.core.state import StateMachine, State
from voiceflow.core.commands import CommandRegistry, Command
from voiceflow.commands.aliases import (
...
try:
    from voiceflow.core.actions import Actions, NUMEROS
except ImportError:
    # core.actions has heavy optional deps (pyautogui, pygetwindow, etc.)
    # and is not yet migrated to the package. Fall back to the old location.
    try:
        from core.actions import Actions, NUMEROS  # type: ignore[no-redef]
    except ImportError:
        Actions = None  # type: ignore[assignment,misc]
        NUMEROS = {}  # type: ignore[assignment]

try:
    from voiceflow.config import VF_HOME
    BASE_DIR = str(VF_HOME)
except ImportError:
    try:
        from config.settings import BASE_DIR  # type: ignore[no-redef]
    except ImportError:
        import os as _os
...
def load_custom_commands(
    ...
    from voiceflow.commands.custom import CustomCommandLoader

    # Use VF_HOME / "commands" as the default commands directory
    try:
        from voiceflow.config import VF_HOME
        commands_dir = str(VF_HOME / "commands")
    except ImportError:
