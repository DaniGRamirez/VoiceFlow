...
from voiceflow.core.commands import Command
from voiceflow.core.state import State
    ...
    def __init__(self, commands_dir: Optional[str] = None, allow_dangerous: bool = False,
        ...
        if commands_dir is None:
            try:
                ...
                commands_dir = os.path.join(os.path.expanduser("~"), ".voiceflow", "commands")

        self.commands_dir = commands_dir

        # Lazy import: core.action_executor has heavy deps (pyautogui, etc.)
        # and is not yet migrated to the package.
        try:
            from voiceflow.core.action_executor import ActionExecutor
        except ImportError:
            try:
                from core.action_executor import ActionExecutor  # type: ignore[no-redef]
            except ImportError:
                ActionExecutor = None  # type: ignore[assignment,misc]

        if ActionExecutor is None:
            raise ImportError(
                "ActionExecutor not available. "
                "Make sure core.action_executor or voiceflow.core.action_executor is importable."
            )
